...
struct ImVec2
{
    ...
    inline ImVec2 operator+(const float& v) const
    {
...
namespace ImGui
{
    ...
    // - Time are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move by 5 pixels to increase value by 1). For gamepad/keyboard navigation, minimum speed is Max(v_speed, minimum_step_at_given_precision).
    ...
    bool SliderScalarCustom(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max,
                       const char *format, ImGuiSliderFlags flags);

    bool
    SliderFloatCustom(const char *label, float *v, float v_min, float v_max, const char *format,
                      ImGuiSliderFlags flags);

    bool ComboCustom(const char *label, int *current_item, const char *items_separated_by_zeros, int height_in_items);

    bool ComboCustom(const char *label, int *current_item, bool (*items_getter)(void *, int, const char **), void *data,
                     int items_count, int popup_max_height_in_items);

    bool BeginComboCustom(const char *label, const char *preview_value, ImGuiComboFlags flags);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_Blur                   = 1 << 21,  // Added by Flash, Adds background blur to a window
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_RootWindow                    = 1 << 1,   // OnRender from root window (top most parent of the current hierarchy)
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_RootWindow                    = 1 << 1,   // IsWindowHovered() only: OnRender from root window (top most parent of the current hierarchy)
...
struct ImColor
{
    ...
    // Divide operator:
    inline ImColor operator/ (const float rhs) const                { return ImColor(Value.x / rhs, Value.y / rhs, Value.z / rhs, Value.w); }
    ...
    __forceinline ImColor Lerp(ImColor target, float pct) {
        return ImColor(ImVec4((this->Value.x + (target.Value.x - this->Value.x) * pct), (this->Value.y + (target.Value.y - this->Value.y) * pct), (this->Value.z + (target.Value.z - this->Value.z) * pct), 255));
    }

    __forceinline ImColor LerpWithOpacity(ImColor target, float pct) {
        return ImColor(ImVec4((this->Value.x + (target.Value.x - this->Value.x) * pct), (this->Value.y + (target.Value.y - this->Value.y) * pct), (this->Value.z + (target.Value.z - this->Value.z) * pct), (this->Value.w + (target.Value.w - this->Value.w) * pct)));
    }
};
